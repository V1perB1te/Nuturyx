  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="lg:hidden sticky top-0 z-20 flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 shadow-sm">
        <BrandBadge subtitle={profile.nombre} iconClassName="h-10 w-10" />
        <button
          type="button"
          onClick={() => setMobileOpen(true)}
          className="flex h-12 w-12 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-700 active:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2"
        >
        ...
        <div className="fixed inset-0 z-30 lg:hidden flex">
          ...
          <aside className="relative flex h-full w-[80%] max-w-sm flex-col bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-100 p-4">
              <BrandBadge subtitle={profile.rol} />
              <button 
                type="button" 
                onClick={() => setMobileOpen(false)} 
                className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 text-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
      ...
      <div className="mx-auto flex w-full max-w-[1400px] gap-4 px-2 py-4 lg:px-6 lg:py-6">
        <aside className={`hidden lg:flex lg:sticky lg:top-6 lg:h-[calc(100vh-3rem)] lg:flex-col lg:rounded-2xl lg:border lg:border-slate-200 lg:bg-white lg:p-4 lg:shadow-sm transition-all duration-300 ${collapsed ? "lg:w-24" : "lg:w-64"}`}>
          <div className="mb-6 flex items-center justify-between">
            {!collapsed ? (
              <BrandBadge subtitle={profile.nombre} />
            ) : (
              <BrandBadge compact />
            )}
            ...
            {!collapsed && (
              <button 
                ...
                className="flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2"
              >
          ...
          <nav className="space-y-1.5 flex-1">
            {collapsed && (
              <button 
                ...
                className="mx-auto mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-slate-600 hover:bg-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2"
                title="Expandir menú"
              >
